        ...
        describe("著者作成") {
            context("リクエストが妥当な場合") {
                it("作成された著者を返す") {
                    ...
                    every { repository.create("夏目漱石", birthDate) } returns
                        Author(
                            id = 1,
                            name = "夏目漱石",
                            birthDate = birthDate,
            ...
            context("生年月日が現在日の場合") {
                it("作成された著者を返す") {
                    ...
                    every { repository.create("今日 生まれ", today) } returns
                        Author(
                            id = 1,
                            name = "今日 生まれ",
                            birthDate = today,
        ...
        describe("著者更新") {
            context("著者名のみ指定された場合") {
                it("著者名だけを更新する") {
                    ...
                    val service = AuthorService(repository)
                    val birthDate = LocalDate.of(1867, 2, 9)
                    every { repository.findById(1) } returns
                        Author(
                            id = 1,
                            name = "夏目漱石",
                            birthDate = birthDate,
                        )
                    every { repository.update(1, "夏目 金之助", birthDate) } returns
                        Author(
                            id = 1,
                            name = "夏目 金之助",
                            birthDate = birthDate,
            ...
            context("生年月日のみ指定された場合") {
                it("生年月日だけを更新する") {
                    ...
                    every { repository.update(1, "夏目漱石", updatedBirthDate) } returns
                        Author(
                            id = 1,
                            name = "夏目漱石",
